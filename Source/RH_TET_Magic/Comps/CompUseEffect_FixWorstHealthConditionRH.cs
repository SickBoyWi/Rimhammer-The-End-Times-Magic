...
// Taken from vanilla code base. Tweaked a bit to not toss errors.
namespace TheEndTimes_Magic
{
    public class CompUseEffect_FixWorstHealthConditionRH : CompUseEffect
    {
        ...
        public override void DoEffect(Pawn usedBy)
        {
            ...
            else
            {
                ...
                BodyPartRecord biggestMissingBodyPart1 = this.FindBiggestMissingBodyPart(usedBy, this.HandCoverageAbsWithChildren);
                if (biggestMissingBodyPart1 != null)
                {
                    this.Cure(biggestMissingBodyPart1, usedBy);
                }
                else
                {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    BodyPartRecord biggestMissingBodyPart2 = this.FindBiggestMissingBodyPart(usedBy, 0.0f);
                                    if (biggestMissingBodyPart2 != null)
                                    {
                                        this.Cure(biggestMissingBodyPart2, usedBy);
                                    }
                                    else
                                    {
        ...
        private BodyPartRecord FindBiggestMissingBodyPart(Pawn pawn, float minCoverage = 0.0f)
        {
            ...
            {
                if ((double)partsCommonAncestor.Part.coverageAbsWithChildren >= (double)minCoverage && !pawn.health.hediffSet.PartOrAnyAncestorHasDirectlyAddedParts(partsCommonAncestor.Part) && (bodyPartRecord == null || (double)partsCommonAncestor.Part.coverageAbsWithChildren > (double)bodyPartRecord.coverageAbsWithChildren))
