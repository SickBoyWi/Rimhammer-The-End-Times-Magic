...
{
    public class CompUseEffect_UlricArtifactWarhammer : CompUseEffect
    {
        public override void DoEffect(Pawn user)
        {
            base.DoEffect(user);
            
            if (user.IsFaithUserUlric())
            {
                ...
                if (compUser != null)
                {
                    ...
                    {
                        if (pawnAbility.Def.defName.Equals(RH_TET_MagicDefOf.RH_TET_Ulric_Warhammer.defName))
                        { 
                            alreadyHad = true;
                            break;
                        }
                    ...
                    {
                        FaithPower newFaithPower = compUser.FaithData.PowersUlric.Find((Predicate<FaithPower>)(x => x.abilityDef.defName.Equals(RH_TET_MagicDefOf.RH_TET_Ulric_Warhammer.defName)));
                        ...
                        Messages.Message("RH_TET_SigmarArtifactUsed".Translate(user.Name, ("RH_TET_UricWarhammerAbility".Translate() + ".")), (LookTargets)((Thing)user), MessageTypeDefOf.PositiveEvent, true);
                    }
                }
            }
            else
        ...
        {
            if (p.IsFaithUserUlric())
            {
                return base.CanBeUsedBy(p);
            }
            else
            {
                return (AcceptanceReport)"RH_TET_SigmarArtifactCannotUse".Translate("Ulric");
            }
        }
    }
}
