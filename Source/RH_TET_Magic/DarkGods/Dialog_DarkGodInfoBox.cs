...
{
    public class Dialog_DarkGodInfoBox : Window
    {
        ...
        public Dialog_DarkGodInfoBox(DarkGod god)
        {
            this.text = god.Info();
            this.title = god.LabelCap;
            if (buttonAText.NullOrEmpty())
            {
                this.buttonAText = "OK".Translate();
            }
            if (god.Def.Portrait != "")
                this.image = ContentFinder<Texture2D>.Get(god.Def.Portrait);
