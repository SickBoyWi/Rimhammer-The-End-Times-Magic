...
{
    public static class Toils_RecipeMagicBench
    {
        ...
        public static Toil MakeUnfinishedThingIfNeeded()
        {
            ...
            toil.initAction = (Action)(() =>
            {
                ...
                if (!curJob.RecipeDef.UsesUnfinishedThing || curJob.GetTarget(TargetIndex.B).Thing is UnfinishedThing)
                    return;
                List<Thing> ingredients = Toils_RecipeMagicBench.CalculateIngredients(curJob, actor);
                Thing dominantIngredient = Toils_RecipeMagicBench.CalculateDominantIngredient(curJob, ingredients);
        ...
        public static Toil DoRecipeWork()
        {
            ...
            toil.initAction = (Action)(() =>
            {
                Pawn actor = toil.actor;
                Job curJob = actor.jobs.curJob;
                JobDriver_DoBillMagicBench curDriver = (JobDriver_DoBillMagicBench)actor.jobs.curDriver;
            ...
            toil.tickAction = (Action)(() =>
            {
                ...
                else if (actor.needs.TryGetNeed<Need_MagicPool>() != null & actor.needs.TryGetNeed<Need_MagicPool>().CurLevel < .01)
                {
                    Messages.Message((string)"RH_TET_Magic_NoMagicPower".Translate(actor.Name), MessageTypeDefOf.NeutralEvent, true);
                    actor.jobs.EndCurrentJob(JobCondition.InterruptForced, true, true);
                }
                else
                {
                    actor.needs.TryGetNeed<Need_MagicPool>().CurLevel -= .0001f;
            ...
            toil.WithProgressBar(TargetIndex.A, (Func<float>)(() =>
            {
                ...
                UnfinishedThing thing = curJob.GetTarget(TargetIndex.B).Thing as UnfinishedThing;
                return (float)(1.0 - (double)((JobDriver_DoBillMagicBench)actor.jobs.curDriver).workLeft / (double)curJob.bill.recipe.WorkAmountTotal(thing?.Stuff));
            }), false, -0.5f);
        ...
        public static Toil FinishRecipeAndStartStoringProduct()
        {
            ...
            toil.initAction = (Action)(() =>
            {
                ...
                Toils_RecipeMagicBench.ConsumeIngredients(ingredients, curJob.RecipeDef, actor.Map);
          ...
          bool tryStoreInSameStorageIfSpotCantHoldWholeStack = false)
        {
            ...
            toil.initAction = (Action)(() =>
            {
                ...
                else
                {
                    ...
                    if (curJob.def == RH_TET_MagicDefOf.RH_TET_Magic_DoBillMagicBench || curJob.def == JobDefOf.DoBill || curJob.def == JobDefOf.RefuelAtomic || curJob.def == JobDefOf.RearmTurretAtomic)
