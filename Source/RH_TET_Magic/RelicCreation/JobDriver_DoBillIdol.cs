...
{
    public class JobDriver_DoBillIdol : JobDriver
    {
        ...
        protected override IEnumerable<Toil> MakeNewToils()
        {
            ...
            f.FailOnBurningImmobile<JobDriver_DoBillIdol>(TargetIndex.A);
            f.FailOn<JobDriver_DoBillIdol>(() => (this.job.GetTarget(TargetIndex.A).Thing is IBillGiver thing1 && (this.job.bill.DeletedOrDereferenced || !thing1.CurrentlyUsableForBills())));
            ...
            yield return JobDriver_DoBillIdol.JumpToCollectNextIntoHandsForBill(getToHaulTarget, TargetIndex.B);
            ...
            yield return Toils_RecipeIdol.PlaceHauledThingInCell(TargetIndex.C, findPlaceTarget, false, false);
            yield return Toils_Jump.JumpIfHaveTargetInQueue(TargetIndex.B, extract);
            extract = (Toil)null;
            getToHaulTarget = (Toil)null;
            findPlaceTarget = (Toil)null;
            yield return gotoBillGiver;
            yield return Toils_RecipeIdol.MakeUnfinishedThingIfNeeded();
            yield return Toils_RecipeIdol.DoRecipeWork().FailOnDespawnedNullOrForbiddenPlacedThings(TargetIndex.A).FailOnCannotTouch<Toil>(TargetIndex.A, PathEndMode.InteractionCell);
            yield return Toils_RecipeIdol.FinishRecipeAndStartStoringProduct();
            ...
            {
                JobDriver_DoBillIdol jobDriverDoBill = f;
