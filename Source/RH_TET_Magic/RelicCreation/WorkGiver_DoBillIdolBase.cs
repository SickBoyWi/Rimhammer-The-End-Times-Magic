...
{
    public class WorkGiver_DoBillIdolBase : WorkGiver_DoBill
    {
        ...
        private static WorkGiver_DoBillIdolBase.DefCountList availableCounts = new WorkGiver_DoBillIdolBase.DefCountList();
        ...
        private static Job FinishUftJob(Pawn pawn, UnfinishedThing uft, Bill_ProductionWithUft bill)
        {
            ...
            if (job1 != null && job1.targetA.Thing != uft)
                return job1;
            Job job2 = JobMaker.MakeJob(RH_TET_MagicDefOf.RH_TET_Magic_DoBillIdol, (LocalTargetInfo)((Thing)bill.billStack.billGiver));
            job2.bill = (Bill)bill;
            ...
            for (int index = 0; index < giver.BillStack.Count; ++index)
            {
                ...
                if ((bill1.recipe.requiredGiverWorkType == null || bill1.recipe.requiredGiverWorkType == this.def.workType) && (Find.TickManager.TicksGame > bill1.nextTickToSearchForIngredients || FloatMenuMakerMap.makingFor == pawn) && (bill1.ShouldDoNow() && bill1.PawnAllowedToStartAnew(pawn)))
                {
                    ...
                    else
                    {
                        if (bill1 is Bill_ProductionWithUft bill)
                        {
                            if (bill.BoundUft != null)
                            {
                                if (bill.BoundWorker == pawn && pawn.CanReserveAndReach((LocalTargetInfo)(Thing)bill.BoundUft, PathEndMode.Touch, Danger.Deadly, 1, -1, (ReservationLayerDef)null, false) && !bill.BoundUft.IsForbidden(pawn))
                                    return WorkGiver_DoBillIdolBase.FinishUftJob(pawn, bill.BoundUft, bill);
                                continue;
                            }
                            UnfinishedThing uft = WorkGiver_DoBillIdolBase.ClosestUnfinishedThingForBill(pawn, bill);
                            if (uft != null)
                                return WorkGiver_DoBillIdolBase.FinishUftJob(pawn, uft, bill);
                        }
                        List<IngredientCount> ingredientCountList = (List<IngredientCount>)null;
                        if (flag1)
                        {
                            ingredientCountList = WorkGiver_DoBillIdolBase.missingIngredients;
                            ingredientCountList.Clear();
                            WorkGiver_DoBillIdolBase.tmpMissingUniqueIngredients.Clear();
                        }
                        bool? nullable;
                            ...
                            if (nullable.GetValueOrDefault() == flag2 & nullable.HasValue)
                            {
                                foreach (Thing requiredIngredient in billMedical.uniqueRequiredIngredients)
                                {
                                    if (requiredIngredient.IsForbidden(pawn) || !pawn.CanReserveAndReach((LocalTargetInfo)requiredIngredient, PathEndMode.OnCell, Danger.Deadly, 1, -1, (ReservationLayerDef)null, false))
                                        WorkGiver_DoBillIdolBase.tmpMissingUniqueIngredients.Add(requiredIngredient);
                                }
                            }
                        }
                        if (!WorkGiver_DoBillIdolBase.TryFindBestBillIngredients(bill1, pawn, (Thing)giver, this.chosenIngThings, ingredientCountList) || !WorkGiver_DoBillIdolBase.tmpMissingUniqueIngredients.NullOrEmpty<Thing>())
                        {
                            if (FloatMenuMakerMap.makingFor != pawn)
                                bill1.nextTickToSearchForIngredients = Find.TickManager.TicksGame + WorkGiver_DoBillIdolBase.ReCheckFailedBillTicksRange.RandomInRange;
                            else if (flag1)
                            {
                                JobFailReason.Is((string)"MissingMaterials".Translate((NamedArgument)ingredientCountList.Select<IngredientCount, string>((Func<IngredientCount, string>)(missing => missing.Summary)).Concat<string>(WorkGiver_DoBillIdolBase.tmpMissingUniqueIngredients.Select<Thing, string>((Func<Thing, string>)(t => t.Label))).ToCommaList(false, false)), bill1.Label);
                                flag1 = false;
                            }
                        ...
                        {
                            Job job = WorkGiver_DoBillIdolBase.TryStartNewDoBillJob(pawn, bill1, giver, this.chosenIngThings, out Job _, true);
                            this.chosenIngThings.Clear();
                            return job;
                        }
                    }
          ...
          bool dontCreateJobIfHaulOffRequired = true)
        {
            ...
            if (haulOffJob != null & dontCreateJobIfHaulOffRequired)
                return haulOffJob;
            Job job = JobMaker.MakeJob(RH_TET_MagicDefOf.RH_TET_Magic_DoBillIdol, (LocalTargetInfo)((Thing)giver));
        ...
        {
            return WorkGiver_DoBillIdolBase.TryFindBestIngredientsHelper((Predicate<Thing>)(t =>
            {
                ...
                return false;
            }), (Predicate<List<Thing>>)(foundThings => WorkGiver_DoBillIdolBase.TryFindBestIngredientsInSet_NoMixHelper(foundThings, ingredients, chosen, WorkGiver_DoBillIdolBase.GetBillGiverRootCell(ingredientDestination, pawn), false, (List<IngredientCount>)null, (Bill)null)), ingredients, pawn, ingredientDestination, chosen, searchRadius);
        }
        ...
        {
            return WorkGiver_DoBillIdolBase.TryFindBestIngredientsHelper((Predicate<Thing>)(t => WorkGiver_DoBillIdolBase.IsUsableIngredient(t, bill)), (Predicate<List<Thing>>)(foundThings => WorkGiver_DoBillIdolBase.TryFindBestBillIngredientsInSet(foundThings, bill, chosen, WorkGiver_DoBillIdolBase.GetBillGiverRootCell(billGiver, pawn), billGiver is Pawn, missingIngredients)), bill.recipe.ingredients, pawn, billGiver, chosen, bill.ingredientSearchRadius);
        }
...
float searchRadius)
    {
        chosen.Clear();
            WorkGiver_DoBillIdolBase.newRelevantThings.Clear();
        if (ingredients.Count == 0)
            return true;
        Region rootReg = WorkGiver_DoBillIdolBase.GetBillGiverRootCell(billGiver, pawn).GetRegion(pawn.Map, RegionType.Set_Passable);
        if (rootReg == null)
            return false;
            WorkGiver_DoBillIdolBase.relevantThings.Clear();
            WorkGiver_DoBillIdolBase.processedThings.Clear();
        bool foundAll = false;
        ...
        {
                WorkGiver_DoBillIdolBase.relevantThings.AddRange((IEnumerable<Thing>)buildingMechGestator.innerContainer);
            if (foundAllIngredientsAndChoose(WorkGiver_DoBillIdolBase.relevantThings))
            {
                    WorkGiver_DoBillIdolBase.relevantThings.Clear();
                return true;
            }
        ...
        int regionsProcessed = 0;
            WorkGiver_DoBillIdolBase.processedThings.AddRange<Thing>(WorkGiver_DoBillIdolBase.relevantThings);
        int num = foundAllIngredientsAndChoose(WorkGiver_DoBillIdolBase.relevantThings) ? 1 : 0;
        RegionProcessor regionProcessor = (RegionProcessor)(r =>
        {
            ...
            for (int index = 0; index < thingList.Count; ++index)
            {
                ...
                if (!WorkGiver_DoBillIdolBase.processedThings.Contains(thing) && ReachabilityWithinRegion.ThingFromRegionListerReachable(thing, r, PathEndMode.ClosestTouch, pawn) && (baseValidator(thing) && !(thing.def.IsMedicine & billGiverIsPawn)))
                {
                    WorkGiver_DoBillIdolBase.newRelevantThings.Add(thing);
                    WorkGiver_DoBillIdolBase.processedThings.Add(thing);
                }
            }
            ++regionsProcessed;
            if (WorkGiver_DoBillIdolBase.newRelevantThings.Count > 0 && regionsProcessed > adjacentRegionsAvailable)
            {
                WorkGiver_DoBillIdolBase.relevantThings.AddRange((IEnumerable<Thing>)WorkGiver_DoBillIdolBase.newRelevantThings);
                WorkGiver_DoBillIdolBase.newRelevantThings.Clear();
                if (foundAllIngredientsAndChoose(WorkGiver_DoBillIdolBase.relevantThings))
                {
                    foundAll = true;
    ...
    private static IntVec3 GetBillGiverRootCell(Thing billGiver, Pawn forPawn)
          ...
          List<IngredientCount> missingIngredients)
        {
            if (bill.recipe.allowMixingIngredients)
                return WorkGiver_DoBillIdolBase.TryFindBestBillIngredientsInSet_AllowMix(availableThings, bill, chosen, rootCell, missingIngredients);
            return WorkGiver_DoBillIdolBase.TryFindBestBillIngredientsInSet_NoMix(availableThings, bill, chosen, rootCell, alreadySorted, missingIngredients);
        }
        ...
        private static bool TryFindBestBillIngredientsInSet_NoMix(
          ...
          List<IngredientCount> missingIngredients)
        {
            return WorkGiver_DoBillIdolBase.TryFindBestIngredientsInSet_NoMixHelper(availableThings, bill.recipe.ingredients, chosen, rootCell, alreadySorted, missingIngredients, bill);
        }
          ...
          Bill bill = null)
        {
            ...
            chosen.Clear();
            WorkGiver_DoBillIdolBase.availableCounts.Clear();
            missingIngredients?.Clear();
            WorkGiver_DoBillIdolBase.availableCounts.GenerateFrom(availableThings);
            for (int index1 = 0; index1 < ingredients.Count; ++index1)
            {
                IngredientCount ingredient = ingredients[index1];
                bool flag = false;
                for (int index2 = 0; index2 < WorkGiver_DoBillIdolBase.availableCounts.Count; ++index2)
                {
                    float f = bill != null ? (float)ingredient.CountRequiredOfFor(WorkGiver_DoBillIdolBase.availableCounts.GetDef(index2), bill.recipe, bill) : ingredient.GetBaseCount();
                    if ((bill == null || bill.recipe.ignoreIngredientCountTakeEntireStacks || (double)f <= (double)WorkGiver_DoBillIdolBase.availableCounts.GetCount(index2)) && ingredient.filter.Allows(WorkGiver_DoBillIdolBase.availableCounts.GetDef(index2)) && (bill == null || ingredient.IsFixedIngredient || bill.ingredientFilter.Allows(WorkGiver_DoBillIdolBase.availableCounts.GetDef(index2))))
                    {
                        for (int index3 = 0; index3 < availableThings.Count; ++index3)
                        {
                            if (availableThings[index3].def == WorkGiver_DoBillIdolBase.availableCounts.GetDef(index2))
                            {
                                ...
                                if (num > 0)
                                {
                                    ...
                                    {
                                        flag = true;
                                        float val = WorkGiver_DoBillIdolBase.availableCounts.GetCount(index2) - f;
                                        WorkGiver_DoBillIdolBase.availableCounts.SetCount(index2, val);
                                        break;
                                    }
                                }
                            }
