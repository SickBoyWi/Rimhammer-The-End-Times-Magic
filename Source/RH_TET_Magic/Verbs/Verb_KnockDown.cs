...
{
    public class Verb_KnockDown : Verb_UseAbility
    {
        protected override bool TryCastShot()
        {
            ...
            {
                if (mp.abilityDef.defName.Contains("VisionOfTorment"))
                {
                    magicPower = mp;
                }
            }
            ...
            if (magicPower == null)
            {
                Log.Error("Someone tried to cast Vision of Torment, but didn't have the spell.");
                return false;
            }
            ...
            HediffDef hediffDef = RH_TET_MagicDefOf.RH_TET_Magic_Knockdown;
            ...
            for (int i = 0; i < this.TargetsAoE.Count; ++i)
            {
                ...
                if (target != null)
                {
                    FleckMaker.Static(target.Position, map, RH_TET_MagicDefOf.RH_TET_FleckPurpleEffect, 1.5f);
                    FleckMaker.Static(target.Position, map, FleckDefOf.PsycastAreaEffect, .2f);

                    target.health.AddHediff(hediffDef);
                    target.health.hediffSet.GetFirstHediffOfDef(hediffDef).Severity = (float)(RH_TET_MagicMod.random.Next(55, 85) * .01f);
