...
{
    public class Verb_Pestilence : Verb_UseAbility
    {
        protected override bool TryCastShot()
        {
            ...
            foreach (MagicPower mp in this.CasterPawn.GetComp<CompMagicUser>().MagicData.PowersNurgle)
            {
                if (mp.abilityDef.defName.Contains("NurglePestilence"))
                {
                    magicPower = mp;
                }
            }
            ...
            if (magicPower == null)
            {
                Log.Error("Someone tried to cast Sumptuous Pestilence, but didn't have the spell.");
                return false;
            }

            Map map = theCaster.Map;
            MoteMaker.MakeStaticMote(caster.Position, map, RH_TET_MagicDefOf.RH_TET_Mote_Afflictions, 2f);
            ...
            //if (rand < 5)
            //    disease = HediffDefOf.Flu;
            //else if (rand < 10)
            //    disease = HediffDefOf.Malaria;
            //else if (rand < 15)
            //    disease = HediffDefOf.FoodPoisoning;
            ...
            for (int i = 0; i < this.TargetsAoE.Count; ++i)
            {
                ...
                if (target != null)
                {
                    MoteMaker.MakeStaticMote(target.Position, map, RH_TET_MagicDefOf.RH_TET_Mote_Afflictions, 1.5f);
                    FleckMaker.Static(target.Position, map, FleckDefOf.PsycastAreaEffect, .2f);

                    target.health.AddHediff(disease);
                    target.health.hediffSet.GetFirstHediffOfDef(disease).Severity = (float)(RH_TET_MagicMod.random.Next(55, 85) * .01f);
