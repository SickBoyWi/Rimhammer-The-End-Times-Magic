...
{
    public class Verb_PhoenixFlames : Verb_UseAbility
    {
        protected override bool TryCastShot()
        {
            ...
            {
                if (mp.abilityDef.defName.Contains("PhoenixFlames"))
                {
                    magicPower = mp;
                }
            }
            ...
            if (magicPower == null)
            {
                Log.Error("Someone tried to cast Flames of the Phoenix, but didn't have the spell.");
                return false;
            }
            ...
            for (int i = 0; i < this.TargetsAoE.Count; ++i)
            {
                ...
                if (target != null)
                {
                    FleckMaker.Static(target.Position, target.Map, RH_TET_MagicDefOf.RH_TET_FleckWhiteEffect, .5f);
