...
{
    public class Verb_RedPox : Verb_UseAbility
    {
        protected override bool TryCastShot()
        {
            ...
            {
                if (mp.abilityDef.defName.Contains("RedPox"))
                {
                    magicPower = mp;
                }
            }
            ...
            if (magicPower == null)
            {
                Log.Error("Someone tried to cast Great Red Pox, but didn't have the spell.");
                return false;
            }
            ...
            FleckMaker.Static(casterLocation, map, RH_TET_MagicDefOf.RH_TET_FleckBrownEffect, .5f);
            FleckMaker.Static(casterLocation, map, RH_TET_MagicDefOf.RH_TET_FleckRedEffect, 2f);
            ...
            for (int i = 0; i < this.TargetsAoE.Count; ++i)
            {
                ...
                if (target != null)
                {
                    FleckMaker.Static(target.Position, map, RH_TET_MagicDefOf.RH_TET_FleckBrownEffect, .5f);
                    FleckMaker.Static(target.Position, map, RH_TET_MagicDefOf.RH_TET_FleckRedEffect, 1f);
