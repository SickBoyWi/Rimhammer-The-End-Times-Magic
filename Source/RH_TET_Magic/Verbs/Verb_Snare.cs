...
{
    public class Verb_Snare : Verb_UseAbility
    {
        protected override bool TryCastShot()
        {
            ...
            foreach (MagicPower mp in this.CasterPawn.GetComp<CompMagicUser>().MagicData.PowersLight)
            {
                if (mp.abilityDef.defName.Contains("Snare"))
                {
                    magicPower = mp;
                }
            }
            ...
            if (magicPower == null)
            {
                Log.Error("Someone tried to cast Abulla's Snare, but didn't have the spell.");
                return false;
            }

            Map map = theCaster.Map;
            FleckMaker.Static(caster.Position, map, RH_TET_MagicDefOf.RH_TET_FleckWhiteEffect, 2f);
            ...
            for (int i = 0; i < this.TargetsAoE.Count; ++i)
            {
                ...
                if (target != null)
                {
                    FleckMaker.Static(target.Position, map, RH_TET_MagicDefOf.RH_TET_FleckWhiteEffect, 1.5f);
                    FleckMaker.Static(target.Position, map, FleckDefOf.PsycastAreaEffect, 1f);
                    target.health.AddHediff(RH_TET_MagicDefOf.RH_TET_LightSnare);
                    maxTargets--;
                    flag1 = true;
                    continue;
                }
