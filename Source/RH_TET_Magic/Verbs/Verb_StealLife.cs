...
{
    public class Verb_StealLife : Verb_UseAbility
    {
        protected override bool TryCastShot()
        {
            ...
            {
                if (mp.abilityDef.defName.Contains("StealLife"))
                {
                    spellLevel = mp.level;
                }
            }
            ...
            if (magicPower == null)
            {
                Log.Error("Someone tried to cast Steal Life but didn't have the spell.");
                return false;
            }
            ...
            for (int i = 0; i < this.TargetsAoE.Count; ++i)
            {
                ...
                if (resultHediffs.TryRandomElement<Hediff_Injury>(out result))
                {
                    ...
                    string text = "RH_TET_MessageStealLifeSpellSuccess".Translate(theCaster.Name, thing.Name).CapitalizeFirst();
