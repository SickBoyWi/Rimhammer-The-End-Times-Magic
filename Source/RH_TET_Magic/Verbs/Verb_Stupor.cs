...
{
    public class Verb_Stupor : Verb_UseAbility
    {
        protected override bool TryCastShot()
        {
            ...
            foreach (MagicPower mp in this.CasterPawn.GetComp<CompMagicUser>().MagicData.PowersSlaanesh)
            {
                if (mp.abilityDef.defName.Contains("SlothfulStupor"))
                {
                    magicPower = mp;
                }
            }
            ...
            if (magicPower == null)
            {
                Log.Error("Someone tried to cast Sltohful Stupor, but didn't have the spell.");
                return false;
            }

            Map map = theCaster.Map;
            FleckMaker.Static(caster.Position, map, RH_TET_MagicDefOf.RH_TET_FleckPinkEffect, 2f);
            ...
            for (int i = 0; i < this.TargetsAoE.Count; ++i)
            {
                ...
                if (target != null)
                {
                    FleckMaker.Static(target.Position, map, RH_TET_MagicDefOf.RH_TET_FleckPinkEffect, 1.5f);
                    FleckMaker.Static(target.Position, map, FleckDefOf.PsycastAreaEffect, .2f);
                    target.health.AddHediff(RH_TET_MagicDefOf.RH_TET_SlaaneshLethargy);
                    maxTargets--;
                    flag1 = true;
                    continue;
                }
