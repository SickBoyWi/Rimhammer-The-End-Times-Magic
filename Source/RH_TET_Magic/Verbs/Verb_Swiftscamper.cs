...
{
    public class Verb_Swiftscamper : Verb_UseAbility
    {
        protected override bool TryCastShot()
        {
            ...
            {
                if (mp.abilityDef.defName.Contains("Swiftscamper"))
                {
                    magicPower = mp;
                }
            }
            ...
            if (magicPower == null)
            {
                Log.Error("Someone tried to cast Swiftscamper, but didn't have the spell.");
                return false;
            }
            ...
            for (int i = 0; i < this.TargetsAoE.Count; ++i)
            {
                ...
                if (target != null)
                {
                    FleckMaker.Static(target.Position, map, RH_TET_MagicDefOf.RH_TET_FleckGreenEffect, 1.5f);
                    FleckMaker.Static(target.Position, map, FleckDefOf.PsycastAreaEffect, 1f);
                    target.health.AddHediff(RH_TET_MagicDefOf.RH_TET_StealthSwiftscamper);
                    maxTargets--;
                    flag1 = true;
                    continue;
                }
